...
export function openOnboarding({ dataDir, root, store, getUrl, servicePort = 0, installerOptions = {} }) {
  ...
  return {
    ...
    async install(body) {
      ...
      // A saved profile may point at a package generated by an earlier release.
      // Rebuild its package from this service while keeping the profile/session identity.
      const prepared = await prepareIntegration({ root, dataDir, url: getUrl(), boardInstanceId: store.boardInstanceId, profile, servicePort, ...installerOptions });
      Object.assign(profile, prepared);
      const result = await installIntegration(profile, installerOptions);
      return transaction(state => { const item = state.profiles.find(p => p.id === profile.id); Object.assign(item, prepared, result); return publicProfile(item); });
    },
    async enable(body) {
    ...
    async agentStatus(body) {
      const profile = await getProfile(body);
      const session = body.session?.sessionId ? profile.sessions[digest(body.session.sessionId)] || null : null;
      const board = await store.read();
      const binding = (board.agentBindings ?? []).find(item => item.host === profile.host
        && item.profileId === profile.id && item.sessionId === body.session?.sessionId);
      const progressCheckpoint = profile.scope === 'project' && binding?.projectId !== profile.projectId
        ? null : getProgressCheckpoint(board, binding, { interrupted: session?.interrupted === true });
      return { session, scope: profile.scope, projectId: profile.projectId, enabled: profile.enabled, progressCheckpoint };
    },
    async verify(body) {
      const profile = await getProfile(body);
