...
- **任何 Agent 都能接。** 提供 Codex、Claude Code 的本机接入包，其它宿主走通用
...
- **不启动也不调度 Agent。** `start` 和 `stop` 只写回执，真正的启动和停止由宿主
  （Claude Code、Codex……）完成。在节点上写了执行者或模型名，不代表
  有任何东西在运行。
- **不替你选模型，也不常驻唤醒器。**
...
**Hook 会话事件送不到看板(v0.1.1 起)。** Hook 回写会话缓存时动态 import 了自己，
而宿主正是把它当入口运行：入口还停在顶层 await 上，等的恰好是这次回写，两边互等到 2 秒网络预算用完。
结果是 hook 报超时、进入 60 秒离线冷却，会话事件始终到不了看板，
接入界面一直显示「尚未收到原生会话入口」。机制与操作系统无关，v0.1.1 和 v0.1.2 都受影响。

v0.1.3 的发布页附带 Windows(x64)与 macOS(Apple 芯片)桌面包。
更喜欢原来的配色？[经典配色版](https://github.com/jarviseven07-prog/jarvisync/releases/tag/v0.1.3-classic)与 v0.1.3 功能完全相同，只保留了墨水屏改版之前的原版配色。
...
**Windows x64 便携版：**从[v0.1.3 发布页](https://github.com/jarviseven07-prog/jarvisync/releases/tag/v0.1.3)
下载 `JarviSync-v0.1.3-windows-x64.zip`，完整解压后打开其中的 `JarviSync.exe`，保留同目录的其它文件。
便携版自带运行时，无需另装 Node.js；程序未签名，首次打开若出现 SmartScreen 提示，点「更多信息 → 仍要运行」。
